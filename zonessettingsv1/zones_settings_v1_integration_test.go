	...
	Describe(`zone_settings_v1_test`, func() {
		Context(`zone_settings_v1_test`, func() {
			...
			It(`Security level setting test`, func() {
				shouldSkipTest()
				getOpt := service.NewGetSecurityLevelOptions()
				getResult, getResp, getErr := service.GetSecurityLevel(getOpt)
				Expect(getErr).To(BeNil())
				Expect(getResp).ToNot(BeNil())
				Expect(getResult).ToNot(BeNil())
				Expect(*getResult.Success).Should(BeTrue())

				updateOpt := service.NewUpdateSecurityLevelOptions()
				updateOpt.SetHeaders(map[string]string{"key": "name"})
				updateOpt.SetValue(UpdateSecurityLevelOptions_Value_Medium)
				updateResult, updateResp, updateErr := service.UpdateSecurityLevel(updateOpt)
				Expect(updateErr).To(BeNil())
				Expect(updateResp).ToNot(BeNil())
				Expect(updateResult).ToNot(BeNil())
				Expect(*updateResult.Success).Should(BeTrue())
				Expect(*updateResult.Result.Value).Should(Equal(UpdateSecurityLevelOptions_Value_Medium))
			})
		})
	})
})
